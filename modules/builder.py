...
def build():
    ...
    if not o.hostname in '!':
        ...
        if o.add_dhcp_hostname:
            ...
            tmp0.write(ifcfgDhcpHostnameScript.encode('utf-8'))
            tmp0.flush()
    ...
    tmp1.write(firstBootScriptStart.encode('utf-8'))
    if o.ssh_pubkey:
        for item in o.ssh_pubkey:
            for keyfile in glob.glob(os.path.expanduser(item)):
                with open(keyfile, 'r') as f:
                    tmp1.write(f.read().encode('utf-8'))
    tmp1.write(firstBootScriptEnd.encode('utf-8'))
    tmp1.flush()
