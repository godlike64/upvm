...
def check_prompt_root_pw():
    if not cfg.opts.root_password:
        if stdout.isatty():
            ...
            while True:
                passwd = input(c.CYAN("Enter root password for new VM or enter '") + c.BOLD("random") + c.CYAN("' or '") + c.BOLD("disabled") + c.CYAN("' or file path : "))
                if passwd:
                    break
            ...
            save_passwd = input(c.CYAN("Save password choice as default to '{}'? ".format(cfg.cfgfileUser)) + c.BOLD("[y]/n") + c.CYAN(" : "))
            if save_passwd != 'n':
...
def check_prompt_vm_name():
    if cfg.opts.vmname:
        ...
        if stdout.isatty():
            ...
            cfg.opts.vmname = input(c.CYAN("Enter VMNAME ") + c.BOLD("[{}]".format(_default_name)) + c.CYAN(" : "))
    ...
    while True:
        ...
        if match:
            ...
            if not stdout.isatty():
                exit(1)
            cfg.opts.vmname = input(c.CYAN("Enter a unique VM name : "))
        else:
            break
...
def check_prompt_hostname():
    if not cfg.opts.hostname:
        ...
        if cfg.opts.hostname_prompt and stdout.isatty():
            cfg.opts.hostname = input(c.CYAN("Enter HOSTNAME ") + c.BOLD("[{}]".format(_default_name)) + c.CYAN(" or ") + c.BOLD("!") + c.CYAN(" to skip changing hostname : "))
            if not cfg.opts.hostname:
